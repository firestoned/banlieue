...
| [70](.github/community/70-ephemeral-vm-pools.md) | Ephemeral single-use VM pools (AI agent sandboxes) | 🔶 | Phase D (`LibvirtMachine`, ADR-0050) done and validated end-to-end against a real cluster + libvirt host; B1 `VirtualMachinePool` (ADR-0046) landed. Open: `VirtualMachineClaim` (B2/ADR-0047), `GuestReady` (A2/ADR-0043) — until which a pool can only use `readiness: InfrastructureReady`, wrong for Deferred images — and the section-0 slim-image experiment |
