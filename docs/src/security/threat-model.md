...
| Claim consumer / sandbox broker | Creates `VirtualMachineClaim`s, holding `create` on them in a namespace | **Untrusted for the accuracy of `spec.subject`** — nothing currently pins it to the caller (§7.10, §8) |
...
| **A claim attributes a sandbox to a subject that never requested one** | S, R | **No technical control yet.** ADR-0047 Decision 10 specifies a `ValidatingAdmissionPolicy` pinning `spec.subject` to the authenticated caller; it is not written. Recorded in §8 with a *Revisit when*, and §7.10 bounds it with RBAC in the meantime |
| A credential is written into `spec.subject`, which is world-readable in the namespace and copied onto the member | I | Documentation-only: the CRD schema, `examples/19-virtualmachineclaim.yaml` and the claims guide all state that `subject` is an identifier. banlieue never reads it as a credential and never forwards it to a guest — but nothing rejects one. See §7.10 |
