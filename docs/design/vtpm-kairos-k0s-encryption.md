...
| Project | Repo | Role |
|---|---|---|
| **[Kairos](https://kairos.io)** | upstream, [kairos-io/kairos](https://github.com/kairos-io/kairos) | The immutable Linux OS distribution every node in this doc boots — read-only root filesystem, atomic upgrades, and `kcrypt` (§4), its built-in TPM-sealed disk-encryption layer. Not banlieue/sceau code; consumed as a base image and an installer. |
| **[k0s](https://k0sproject.io)** | upstream, [k0sproject/k0s](https://github.com/k0sproject/k0s) | The single-binary Kubernetes distribution every control-plane node runs. Owns `kube-apiserver` and `etcd`; §5's `sceau` plugs into k0s's own `EncryptionConfiguration`/KMSv2 support to seal Secret values, and §8's HA design works around k0s's multi-controller topology. Not banlieue/sceau code. |
| **banlieue** | [`firestoned/banlieue`](https://github.com/firestoned/banlieue) (public OSS on the maintainer's GitHub, no organization-specific content) | Kubernetes-native VM lifecycle API. `VirtualMachine`/`VMClass`/`VMImage`/`Provider` CRDs; schedules VMs onto vSphere via [Cluster API](https://github.com/kubernetes-sigs/cluster-api) (CAPI — the upstream Kubernetes SIG project for declarative, provider-agnostic cluster/machine lifecycle management)-style infra CRDs (`VSphereMachine`). |
...
| **image pipeline** | private CI (GitHub Actions) | Builds the Kairos container images consumed by `OSArtifact` builds — a glibc (RHEL-based) variant and a musl (Alpine-based) variant — drives ISO creation via [Auroraboot](https://github.com/kairos-io/AuroraBoot) (Kairos's own image-building tool), and clones vSphere templates. Owns the base cloud-config layer every node boots with. Not banlieue code and not open-sourced; treated here as an external input, same as kairos-operator. |
...
There is also a machine-readable [FINOS CALM](https://calm.finos.org)
(Common Architecture Language Model — an open-source, JSON-based standard
from the [FINOS](https://www.finos.org) financial-services open-source
foundation for describing system architecture in a format tools can lint,
diff, and render diagrams from, instead of only free-hand prose/diagrams)
model of this same architecture at
...
1. **Template creation, one-time, out of band, before banlieue exists at
   all.** The image pipeline's build scripts (§1) call [`govc`](https://github.com/vmware/govmomi/tree/main/govc)
   (VMware's own open-source vSphere/vCenter CLI, built on the `govmomi` Go
   SDK) directly:
   ```console
...
Provider (Configure → Key Providers) — in this environment, a
[KMIP](https://www.oasis-open.org/committees/kmip/)-backed
(Key Management Interoperability Protocol — an OASIS open standard for
talking to an external key-management appliance) provider fronting a
Thales CipherTrust/KeySecure server. This KMS governs
...
Kairos's [`kcrypt`](https://github.com/kairos-io/kairos) component (now
folded into the main `kairos-io/kairos` monorepo) can LUKS-encrypt
`COS_PERSISTENT` and seal the
...
- **Language/libraries:** Rust — `tonic`/`prost` generate the KMSv2 server
  ...
  [`tss-esapi`](https://github.com/parallaxsecond/rust-tss-esapi) (safe Rust
  bindings for the TCG's TPM2 Software Stack Enhanced System API) provides
  safe TPM calls.
