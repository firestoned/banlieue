...
4. **Undefine always passes `MANAGED_SAVE | NVRAM | TPM`** (`1<<0 | 1<<2 |
...
5. **Domain XML is built by a dedicated `xml` module with mandatory
...
6. **Two disk shapes, chosen by install mode**, mirroring what ADR-0040
   forces on vSphere:
...
7. **Address discovery is source-ordered, not source-fixed**:
...
8. **Controller dispatch by provider class.**
...
9. **vTPM is out of scope here** and is ADR-0051. This ADR gets a VM booting
...
10. **Live migration stays out.** `migrationPolicy` other than `Never`
