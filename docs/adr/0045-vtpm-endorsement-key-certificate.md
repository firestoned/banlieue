...
`GuestReady`, on both backends.** Exactly ADR-0044's ordering and for exactly its reason: a
...
vSphere gates it in `status_with_observed_state` and libvirt in
`build_status`, both with reason `TpmEndorsementPending`. That symmetry only
became possible when ADR-0043's vSphere transport landed and gave vSphere a
`GuestReady` to gate; before it, vSphere had no such condition at all.
...
**The vSphere read retries; it is not a one-shot.** It lives in
`refresh_power_state`, which `reconcile` sends every provisioned machine
through, so an empty answer from vCenter is a delay rather than a permanent
absence — re-read until something is recorded, then sticky. `ensure_vm` reads
it too, through the same helper, so a synchronously-issued certificate is
recorded before the guest boots; nothing depends on that being the case.

**vSphere's half is not yet verified live.** Decision 4 ships as code with
unit tests. ADR-0043's vSphere transport has since been exercised against a
real vCenter, so the environment to close this now exists — it simply has not
been run for this field yet. The libvirt path is verified end-to-end against
a real host (`make libvirt-ek-live-test`).

**A pre-existing SSA retraction was fixed to make this field durable.**
`patch_status_failed` applied only `{conditions, observedGeneration}` from
the same field manager that applies the whole status elsewhere — the exact
hazard `status_with_observed_state`'s own comment documents having hit live
("the same field manager must always apply the same complete field set"), so
one transient vCenter error retracted `vmRef`, `tpmAttached`, `guestInstalled`
and would have retracted this certificate. It now builds the complete status
through `status_reporting_failure`, like every other `Ready=False` report.
The defect predates this ADR; it is fixed here because an anchor a single
connection blip erases is not an anchor.
