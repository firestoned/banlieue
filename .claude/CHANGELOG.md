...
### Changed
- `.github/workflows/build.yaml`: added `private-repository: true` to the
  `slsa-provenance` job's call into
  `slsa-framework/slsa-github-generator`'s reusable
  `generator_generic_slsa3.yml` workflow.

### Why
The public Build badge on the docs homepage
(https://firestoned.github.io/banlieue/) showed red, which looked wrong
since every actual code job (build, test, clippy-equivalent, security
scans, image builds, signing) was passing on both `main` and the
`tpm-support` branch. Root-caused live via `gh run view`/`gh api .../logs`
on the last failing `main` push (`33829409612`, the userData PR #22
merge): the failure was isolated entirely to the `SLSA Provenance / final`
job, caused by its `generator` job halting early with `Repository is
private. The workflow has halted in order to keep the repository name
from being exposed in the public transparency log.` The repo is actually
public (`gh api repos/firestoned/banlieue --jq .private` → `false`), and
the same job's own log prints `"private": false` moments after the halt
message — confirmed by re-running the failed jobs (`gh run rerun
33829409612 --failed`) and pulling the raw job log, which reproduced the
identical halt on a verified-public repo. This points to the generator's
repo-visibility signal lagging the real GitHub API/OIDC state rather than
an actual privacy concern -- not something fixable from this repo's side
beyond the tool's own documented escape hatch. PR-triggered builds never
hit this at all (`if: github.event_name != 'pull_request'` skips the SLSA
job entirely), which is why `tpm-support`'s own PR runs were green while
...
- [x] Config change only -- CI workflow, no code change. Confirmed the
      ...
      actual build/test/lint/scan/image jobs.
- [ ] Documentation only
