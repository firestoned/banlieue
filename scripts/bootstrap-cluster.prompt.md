...
`/usr/local/bin/k0s`), then MetalLB and flux-operator + a `flux-core`
Kustomization.
...
- **flux** — the OCI repo (`oci://.../flux-core/main`), the cluster
  substitutions (`CLUSTER_DNS`, `CLUSTER_ENV`, appcode), and **who supplies the
  secrets** (`flux-artifactory` dockerconfigjson is required; `flux-tls` can be
  built from the org CA). Never fabricate or copy secrets without approval.
...
`K0S_*`). Save it outside the repo, `chmod 600`.
...
## 5. flux-operator + flux-core

Stage `00-flux-operator` (install), `30-flux-instance` (FluxInstance +
`flux-core` OCIRepository), `40-flux-bootstrap` (the `flux-core` Kustomization
with `postBuild.substitute` for this cluster) into
`/var/lib/k0s/manifests/flux-operator/`. Create `flux-tls` from the org CA; have
the operator supply `flux-artifactory` (registry creds). Once it reconciles,
they add `flux-core-dns` (→ scout → publishes the MetalLB IPs so the API SAN
resolves).
