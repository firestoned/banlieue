...
> **Status (audited against the tree 2026-09-24): the site is built and
> shipping; the information architecture diverged from the plan.** The
> `installation/`, `operations/`, `advanced/` and `development/` sections
> were never created under those names — what the site actually has is
> **Guides** (16 pages, task-shaped, install among them), **Why banlieue?**
> (`reasoning/`, which was not planned at all), **Concepts**,
> **Architecture**, **Developer**, **Security** and **Reference**. Check
> `docs/mkdocs.yml`'s `nav` rather than this list for the current shape.
> Content work continues, which is why roadmap 08 is 🔶 on
> [`ROADMAPS.md`](../../ROADMAPS.md), not ✅.

- [x] ~~Run `poetry init` inside `docs/` and pin the same versions as 5-spot's
      `pyproject.toml` (mkdocs 1.6+/<2.0, mkdocs-material ^9.5, plus the
      five plugins 5-spot uses).~~ **Done** — `docs/pyproject.toml` +
      `docs/poetry.lock`.
- [x] ~~Copy `mkdocs.yml` from 5-spot; rewrite `site_name`, `site_url`,
      `repo_url`, `nav`. Keep theme, palette, features.~~ **Done** —
      `docs/mkdocs.yml`.
- [x] ~~Copy `src/stylesheets/extra.css`; substitute banlieue brand palette
      (see proposal above).~~ **Done** — `docs/src/stylesheets/`.
- [x] ~~Copy `src/javascripts/mermaid-init.js` verbatim.~~ **Done** —
      `docs/src/javascripts/`.
- [x] ~~Author `src/index.md` (tagline + system overview + 3 quick-links).~~
      **Done** — `index.md` plus `overview.md` behind it.
- [x] ~~Author `src/installation/{quickstart,prerequisites,crds}.md` — CRD
      install instructions can use the existing `cargo run --bin crdgen`
      output piped to `kubectl apply`.~~ **Done as `guides/`, not
      `installation/`** — `guides/host-bootstrap.md` and
      `guides/end-to-end-setup.md` carry the install path, and it is
      `banlieue bootstrap` (ADR-0013) rather than piping `crdgen` into
      `kubectl apply`.
- [x] ~~Author `src/concepts/index.md` + one page per CR
      (`provider.md`, `vmclass.md`, `vmimage.md`, `virtualmachine.md`,
      `vsphere-machine.md`). Lean on the rustdoc comments already in
      `crates/banlieue-api/src/`.~~ **Done, grouped rather than
      one-per-kind** — `concepts/{index,architecture,virtualmachine,
      providers,infra-crds-capi,virtualmachine-claim-flow}.md`. Per-kind
      field detail lives in the generated `reference/api.md`, so a
      hand-written page per CR would duplicate it and drift.
- [x] ~~Stub `src/operations/`, `src/advanced/`, `src/security/` — one
      `index.md` per section saying "coming with Phase X".~~ **Superseded**
      — no "coming soon" stubs were shipped. `security/` exists with real
      content (the threat model); `operations/` and `advanced/` material
      went into `guides/` instead.
- [x] ~~Author `src/development/{setup,building,testing,contributing}.md`.~~
      **Partly done, as `developer/`** — `developer/{index,local-development,
      oauth-clients}.md`. A contributing page is still missing, and it is
      the same gap as `CONTRIBUTING.md` in roadmap 12 §4.9.
- [x] ~~Add `src/reference/api.md` pointing at the generated `crds.yaml`
      plus a link to rustdoc.~~ **Done** — regenerated by `make api-docs`,
      which `make docs` runs first.
- [ ] Add `src/changelog.md` (use mkdocs-include-markdown to inline
      `.claude/CHANGELOG.md` — or a symlink if the include plugin is
      excluded for simplicity). **Still open** — and worth deciding
      deliberately: `.claude/CHANGELOG.md` is an internal engineering log
      with `**Author:**` attribution per entry, not a user-facing release
      note. Publishing it verbatim is probably the wrong move; a generated
      release changelog (roadmap 12 §4.8) is the better source.
- [x] ~~Add `src/license.md` — short text + link to `LICENSE`.~~ **Done** —
      at `reference/license.md`.
- [x] ~~Author `src/architecture/system.md` — Mermaid diagram of the CR
      graph (Provider ← VirtualMachine → VMClass/VMImage; VirtualMachine
      → VSphereMachine).~~ **Done** — `architecture/{index,system,flows}.md`,
      and the diagrams are now generated from the CALM model by
      `make calm-diagrams` rather than hand-drawn.
- [x] ~~Add root `Makefile` with the 5 targets above.~~ **Done** —
      `docs`, `docs-serve`, `docs-clean`, `docs-deploy`, `api-docs`.
- [x] ~~Add `.github/workflows/docs.yaml`. Validate locally with `actionlint`.~~
      **Done**.
- [x] ~~Add `docs/site/` and `target/doc/` to `.gitignore`.~~ **Done**.
- [x] ~~Enable GitHub Pages on `firestoned/banlieue` (source: GitHub
      Actions). One-time repo setting outside this checklist.~~ **Done** —
      `docs.yaml` builds and deploys to Pages on merges to `main` (and on a
      release), with `make docs-deploy` (`mkdocs gh-deploy`) as the manual
      escape hatch.
- [x] ~~Smoke test: `make docs-serve`, open the site, click through every
      nav entry, confirm no broken links or styling regressions.~~ **Done**
      — and `docs.yaml` rebuilds the site in CI. Note it is not a link
      check: `mkdocs.yml` does not set `strict: true`, so a dangling link
      warns rather than failing the build. Turning that on is a cheap win
      while the nav keeps growing.
- [x] ~~Smoke test: `make docs`, confirm `docs/site/rustdoc/` and
      `docs/site/reference/crds.yaml` are present.~~ **Done**.
