...
3. **ADR numbers 0041 and 0042 are each used twice** in `docs/adr/`
   (`0041-imagebuilder-namespaced-secret-access` /
   `0041-vmimage-trusted-boot-uki-support`, and
   `0042-instant-clone-vmfork-not-supported` /
   `0042-userdata-reference-authorization`). References to "ADR-0041" and
   "ADR-0042" in this document mean the trusted-boot and instant-clone ones.
   Renumbering the duplicates is tracked in
   [roadmap 40 §4.10](40-PHASE-4-FINOS-READY.md); still open as of
   2026-09-19.

   **Numbers 0043–0049 are reserved by this roadmap** (phase table below) and
   are still free in `docs/adr/`. ADR-0050 is taken (libvirt domain
   lifecycle), so an ADR *unrelated to this roadmap* starts at **0051**.
...
| No Instant Clone. Incompatible with a vTPM on the fork source. | ADR-0042 |
| Trusted Boot/UKI is experimental on vSphere: about 3 minutes of silent stalls per boot. Sandboxes use the classic GRUB image until section 0 says otherwise. | ADR-0041 |
...
> was already the plan (ADR-0041). The sizing in phase B keeps using measured
...
ADR-0041 recorded multi-minute silence at shim → systemd-boot → UKI with no
...
port ADR-0041 already planned, before first power-on:
...
**Variant A, Hadron core.** Note ADR-0041: a Hadron image failed to boot
...
- [ ] Section 0 matrix filled in and linked from ADR-0041.
