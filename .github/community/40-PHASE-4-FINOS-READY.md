...
- [ ] **Renumber the two duplicate ADR numbers.** `0041` and `0042` are each
      used twice (`0041-imagebuilder-namespaced-secret-access` /
      `0041-vmimage-trusted-boot-uki-support`,
      `0042-instant-clone-vmfork-not-supported` /
      `0042-userdata-reference-authorization`). Roadmap 70 references the
      trusted-boot and instant-clone ones by those numbers, so fix the
