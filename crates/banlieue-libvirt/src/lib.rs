...
pub use procs::{
    ...
    DOMAIN_UNDEFINE_MANAGED_SAVE, DOMAIN_UNDEFINE_NVRAM, DOMAIN_UNDEFINE_TPM, DhcpLease, Domain,
    DomainInterface, DomainIpAddr, DomainState, InterfaceAddressSource, NETWORK_DHCP_LEASES_MAX,
    Network, StoragePool, StorageVol, UUID_LEN, VIR_ERR_NO_DOMAIN, VIR_ERR_NO_STORAGE_POOL,
    VIR_ERR_NO_STORAGE_VOL, VOLUME_NAME_MAX, auth_list, connect_open,
    decode_network_get_dhcp_leases_ret, domain_create, domain_define_xml, domain_destroy,
    domain_get_state, domain_interface_addresses, domain_lookup_by_name, domain_shutdown,
    domain_undefine, encode_network_get_dhcp_leases_args, is_not_found, list_all_networks,
    list_all_storage_pools, network_get_dhcp_leases, qcow2_overlay_volume_xml, qcow2_volume_xml,
    raw_volume_xml, storage_pool_list_all_volumes, storage_pool_lookup_by_name,
