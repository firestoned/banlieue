...
mod tests {
    ...
    // ------------------------------------------------------------------
    // UUID — redefining in place
    // ------------------------------------------------------------------

    /// A first define omits `<uuid>` and lets libvirt mint one.
    #[test]
    fn a_first_define_carries_no_uuid() {
        let spec = bios_spec();
        let xml = build_domain_xml(&input(&spec)).unwrap();
        assert!(!xml.contains("<uuid>"), "{xml}");
    }

    /// A redefine must name the UUID already on the host. libvirt matches a
    /// domain by UUID, not by name: without this the define generates a new
    /// UUID and is refused with "already exists with uuid …", which is what
    /// made every second reconcile fail against a real daemon.
    #[test]
    fn a_redefine_carries_the_existing_uuid() {
        let spec = bios_spec();
        let mut i = input(&spec);
        i.uuid = Some("a01fa6a5-6404-406f-8e37-4ed4946c6c18");
        let xml = build_domain_xml(&i).unwrap();
        assert!(
            xml.contains("<uuid>a01fa6a5-6404-406f-8e37-4ed4946c6c18</uuid>"),
            "{xml}"
        );
        // Immediately after <name>, where libvirt's own dumpxml puts it.
    ...
    /// a diagnostic, for when libvirtd accepts a document but the guest
    /// does not behave.
    #[test]
    #[ignore = "diagnostic; run with --nocapture"]
    fn dump_a_bios_domain_for_inspection() {
        let mut spec = bios_spec();
        spec.domain_name = "cirros-seed-demo".to_string();
        spec.memory_mi_b = 512;
        spec.vcpus = 1;
        let mut i = input(&spec);
        i.os_disk_path = "/var/lib/libvirt/pools/images/cirros-seed-demo-os.qcow2";
        i.cidata_iso_path = Some("/var/lib/libvirt/pools/images/cirros-seed-demo-cidata.iso");
        let xml = build_domain_xml(&i).unwrap();
        eprintln!("{}", xml.replace("><", ">\n<"));
    }
}
