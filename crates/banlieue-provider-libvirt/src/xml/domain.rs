...
pub struct DomainXmlInput<'a> {
    ...
    /// The existing domain's UUID, in canonical 8-4-4-4-12 form, when this
    /// machine has already been defined once.
    ///
    /// **Not optional in practice, despite the type.** `DOMAIN_DEFINE_XML`
    /// is *not* an unconditional upsert: libvirt identifies a domain by UUID,
    /// so a document with no `<uuid>` gets a freshly generated one, and the
    /// define then fails with "already exists with uuid …" against the
    /// domain of the same name. Redefining in place requires naming the UUID
    /// that is already there. Found live — every second reconcile failed.
...
pub fn build_domain_xml(input: &DomainXmlInput<'_>) -> Result<String, DomainXmlError> {
    ...
    if let Some(uuid) = input.uuid {
        out.push_str(&format!("<uuid>{}</uuid>", esc(uuid)?));
    }
    out.push_str(&format!(
